...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $guestNumber = sanitize_input($_POST['guest_count']);
    $startTime = sanitize_input($_POST['start_time']);

    // Add 1 hour to the start time to set the finish time
    ...
    if ($finishTime > $maxFinishTime) {
        ...
        $insertReservationQuery = "INSERT INTO reservation (table_number, guest_number, start_time, finish_time) VALUES (?, ?, ?, ?)";
        $stmtReservation = $conn->prepare($insertReservationQuery);
        $stmtReservation->bind_param("isss", $tableNumber, $guestNumber, $startTime, $finishTime);
...
<body>
    ...
    <form method="POST" action="">
        ...
        <br><br>
        <label for="guest_count">Hány személyre számítsunk?</label>
        ...
        <select name="finish_time" id="finish_time" required>
            <option value="" disabled selected></option>
        </select>
        <br><br>
        <input type="submit" value="Lefoglalom">
        <a href="change.php" class="link">Rezerválás módosítása</a>
        <br><br>
        <a href="change_account.php" class="change-link">A fiókom adatainak módosítása</a>
    </form>
